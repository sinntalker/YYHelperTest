...
public class StepService extends Service implements SensorEventListener {
    ...
    private String TAG_StepService = "StepService";
    /**
     * 默认为30秒进行一次存储
     */
    ...
    public void onCreate() {
        super.onCreate();
        Log.d(TAG_StepService, "onCreate()");
        initNotification();
        initTodayData();
    ...
    private void initNotification() {
        ...
        Log.d(TAG_StepService, "initNotification()");
    }
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            Log.v(TAG_StepService, "StepData=" + list.get(0).toString());
            CURRENT_STEP = Integer.parseInt(list.get(0).getStep());
        } else {
            Log.v(TAG_StepService, "出错了！");
        }
        if (mStepCount != null) {
        ...
        mBatInfoReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(final Context context, final Intent intent) {
                ...
                if (Intent.ACTION_SCREEN_ON.equals(action)) {
                    Log.d(TAG_StepService, "screen on");
                } else if (Intent.ACTION_SCREEN_OFF.equals(action)) {
                    Log.d(TAG_StepService, "screen off");
                    //改为60秒一存储
                    duration = 60000;
                } else if (Intent.ACTION_USER_PRESENT.equals(action)) {
                    Log.d(TAG_StepService, "screen unlock");
//                    save();
                    //改为30秒一存储
                    duration = 30000;
                } else if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(intent.getAction())) {
                    Log.i(TAG_StepService, " receive Intent.ACTION_CLOSE_SYSTEM_DIALOGS");
                    //保存一次
                    save();
                } else if (Intent.ACTION_SHUTDOWN.equals(intent.getAction())) {
                    Log.i(TAG_StepService, " receive ACTION_SHUTDOWN");
                    save();
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    private void updateNotification() {
        ...
        Log.d(TAG_StepService, "updateNotification()");
    }
    ...
    private void addBasePedometerListener() {
        ...
        if (isAvailable) {
            Log.v(TAG_StepService, "加速度传感器可以使用");
        } else {
            Log.v(TAG_StepService, "加速度传感器无法使用");
        }
    }
