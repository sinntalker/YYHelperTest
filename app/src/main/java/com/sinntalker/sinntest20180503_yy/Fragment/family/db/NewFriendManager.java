...
import com.sinntalker.sinntest20180503_yy.Fragment.family.db.DaoMaster;
...
public class NewFriendManager {
    ...
    private List<NewFriend> getNoVerifyNewFriend(){
        NewFriendDao dao =  openReadableDb().getNewFriendDao();
        return dao.queryBuilder().where(NewFriendDao.Properties.Status.eq(AddFriendConfig.STATUS_VERIFY_NONE))
                .build().list();
    }
    ...
    public void updateBatchStatus(){
        ...
        if(infos!=null && infos.size()>0){
            ...
            for (int i = 0; i < size; i++) {
                ...
                msg.setStatus(AddFriendConfig.STATUS_VERIFY_READED);
                all.add(msg);
            }
