...
import com.sinntalker.sinntest20180503_yy.Fragment.family.bean.AgreeAddFriendMessage;
import com.sinntalker.sinntest20180503_yy.Fragment.family.db.AddFriendConfig;
...
public class NewFriendAdapter extends BaseRecyclerAdapter<NewFriend> {
    ...
    public NewFriendAdapter(Context context, IMutlipleItem<NewFriend> items, Collection<NewFriend> datas) {
        ...
        public void bindView(final BaseRecyclerHolder holder, final NewFriend add, int position) {
            holder.setImageView(add == null ? null : add.getAvatar(), R.drawable.avatar, R.id.iv_recent_avatar);
            ...
            //当状态是未添加或者是已读未添加
            if (status == null || status == AddFriendConfig.STATUS_VERIFY_NONE || status == AddFriendConfig.STATUS_VERIFY_READED) {
    ...
    private void sendAgreeAddFriendMessage(final NewFriend add, final SaveListener<Object> listener) {
        ...
        final AllUserBean currentUser = BmobUser.getCurrentUser(AllUserBean.class);
        ...
        messageManager.sendMessage(msg, new MessageSendListener() {
            ...
            public void done(BmobIMMessage msg, BmobException e) {
                if (e == null) {//发送成功
                    NewFriendManager.getInstance(context).updateNewFriend(add, AddFriendConfig.STATUS_VERIFIED);
                    listener.done(msg, e);
                } else {//发送失败
                    Logger.e(e.getMessage());
                    listener.done(msg, e);
