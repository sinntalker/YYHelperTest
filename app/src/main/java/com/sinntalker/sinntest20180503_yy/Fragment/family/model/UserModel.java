...
public class UserModel extends BaseModel {
    ...
    public void register(String username, String password, String pwdagain, final LogInListener listener) {
        ...
        final AllUserBean user = new AllUserBean();
        user.setUsername(username);
        user.setPassword(password);
        user.signUp(new SaveListener<AllUserBean>() {
            @Override
            public void done(AllUserBean user, BmobException e) {
                if (e == null) {
    ...
    public void login(String username, String password, final LogInListener listener) {
        ...
        user.setUsername(username);
        user.setPassword(password);
        user.login(new SaveListener<AllUserBean>() {
            @Override
            public void done(AllUserBean user, BmobException e) {
                if (e == null) {
     ...
     * @return
     */
    public AllUserBean getCurrentUser() {
        return BmobUser.getCurrentUser(AllUserBean.class);
    }
     ...
     * @param listener
     */
    public void queryUsers(String username, final int limit, final FindListener<AllUserBean> listener) {
    ...
    public void updateUserInfo(MessageEvent event, final UpdateCacheListener listener) {
        ...
        if (!username.equals(title) || (avatar != null && !avatar.equals(icon))) {
            UserModel.getInstance().queryUserInfo(info.getUserId(), new QueryUserListener() {
                @Override
                public void done(AllUserBean s, BmobException e) {
                    if (e == null) {
                        String name = s.getUsername();
                        String avatar = s.getUserAvatar();
    ...
    //TODO 好友管理：9.12、添加好友
    public void agreeAddFriend(AllUserBean friend, SaveListener<String> listener) {
        Friend f = new Friend();
