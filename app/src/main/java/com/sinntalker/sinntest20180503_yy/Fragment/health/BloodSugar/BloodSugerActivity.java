...
public class BloodSugerActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (type != null && type.length() > 0) {
            //查询条件一：用户名称
            BmobQuery<FamilySugarValueBean> queryUserName = new BmobQuery<FamilySugarValueBean>();
            queryUserName.addWhereEqualTo("user",mCurrentUser);

            //查询条件二：药箱编号
            BmobQuery<FamilySugarValueBean> queryDrugBoxNum = new BmobQuery<FamilySugarValueBean>();
            queryDrugBoxNum.addWhereEqualTo("relations",type);

            //最后查询时完整的条件
            List<BmobQuery<FamilySugarValueBean>> allQueries = new ArrayList<BmobQuery<FamilySugarValueBean>>();
            allQueries.add(queryUserName);
            allQueries.add(queryDrugBoxNum);

            //查询
            BmobQuery<FamilySugarValueBean> query = new BmobQuery<FamilySugarValueBean>();
            query.and(allQueries);
            query.findObjects(new FindListener<FamilySugarValueBean>() {
                @Override
                public void done(List<FamilySugarValueBean> list, BmobException e) {
                    if (e == null) {
...
//                            TextView tv_min= CommonViewHolder.get(item,R.id.id_item_textView_dataMinPressure_historyData);
                                ...
                                tv_Max.setText(strMax[position]);
                                tv_min.setText(strMin[position]);
