...
public class BloodPressureActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (type != null && type.length() != 0) {
            ...
            //查询条件一：用户名称
            BmobQuery<FamilyBloodDataBean> queryUserName = new BmobQuery<FamilyBloodDataBean>();
            queryUserName.addWhereEqualTo("user",user);

            //查询条件二：药箱编号
            BmobQuery<FamilyBloodDataBean> queryDrugBoxNum = new BmobQuery<FamilyBloodDataBean>();
            queryDrugBoxNum.addWhereEqualTo("relations",type);

            //最后查询时完整的条件
            List<BmobQuery<FamilyBloodDataBean>> allQueries = new ArrayList<BmobQuery<FamilyBloodDataBean>>();
            allQueries.add(queryUserName);
            allQueries.add(queryDrugBoxNum);

            //查询
            BmobQuery<FamilyBloodDataBean> query = new BmobQuery<FamilyBloodDataBean>();
            query.and(allQueries);
            query.findObjects(new FindListener<FamilyBloodDataBean>() {
                @Override
                public void done(List<FamilyBloodDataBean> list, BmobException e) {
                    if (e == null) {
